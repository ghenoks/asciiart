...
import exporter.*
import filter.*
import loader.{ImageLoader, JPGFileImageLoader, PNGFileImageLoader, RandomImageLoader, StdFileImageLoader}
...
import models.{Argument, Axis, BusinessError, ModuleHolder}
...
class MyModuleGetter(moduleList: List[Argument]) extends ModuleGetter[ModuleHolder] {
  ...
  override def getModules: Either[BusinessError, ModuleHolder] = {
    for {
      ...
      greyScaleConverter <- getGreyScaleConverter.left.map(error => BusinessError(error.message))
      filter <- getFilters.left.map(error => BusinessError(error.message))
      asciiConverter <- getASCIIConverter.left.map(error => BusinessError(error.message))
      exporter <- getExporter.left.map(error => BusinessError(error.message))
  ...
  private def getLoader: Either[BusinessError, ImageLoader] = {
    ...
    if (imageCommands.isEmpty) Left(BusinessError("No image command specified."))
    else if (imageCommands.length > 1) Left(BusinessError("Multiple image commands specified. Please specify only one image command."))
    else {
          ...
          imageCommand.value match {
            case Some(imagePath) =>
              if (imagePath.endsWith(".jpg"))
                JPGFileImageLoader(imagePath) match {
                  case Right(loader) => Right(loader)
                  case Left(error) => Left(error)
                }
              else if (imagePath.endsWith(".png"))
                PNGFileImageLoader(imagePath) match {
                  case Right(loader) => Right(loader)
                  case Left(error) => Left(error)
                }
              else Left(BusinessError("Unsupported image format. Please use JPG or PNG file."))
            case None =>
              Left(BusinessError("No image path provided for 'image' command."))
          }
        case "image-random" =>
          Right(RandomImageLoader())
        case _ =>
          Left(BusinessError("Unknown image command."))
      }
    }
  }
    ...
    for (command <- filterCommands if error.isEmpty) {
      command match {
        ...
        case Argument("rotate", None) =>
          error = Some("Rotation filter is missing argument.")
        case Argument("rotate", Some(value)) =>
          value match {
            ...
            case _ => error = Some("Invalid argument for Rotation filter.")
          }

        case Argument("brightness", None) =>
          error = Some("Brightness filter is missing argument.")
  ...
  private def getASCIIConverter: Either[BusinessError, ASCIIConverter[GreyScaleImage, GreyScalePixel]] = {
    ...
    if (tableCommands.length > 1) Left(BusinessError("Multiple table commands specified. Please specify only one table command."))
      ...
      tableCommand.name match {
        case "table" =>
          tableCommand.value match {
            ...
            case _ => Left(BusinessError("Missing table info."))
          }
        case "custom-table" =>
          tableCommand.value match {
  ...
  private def getExporter: Either[BusinessError, Exporter[Image]] = {
    ...
    if (exportCommands.isEmpty) {
      return Left(BusinessError("No output command specified."))
    }
    ...
    for (command <- exportCommands if error.isEmpty) {
      command match {
        ...
        case Argument("output-file", None) =>
          error = Some("Output to file command specified without a file path.")
        case _ =>
